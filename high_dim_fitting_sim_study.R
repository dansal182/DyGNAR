#simulate a high-dimensional data scenario for dynamic GNAR
library(GNAR)
library(igraph)
library(fastCorbit)
...
W_USA = weights_matrix_cpp(usa_net_GNAR, 11)
...
for (t in sample_sizes) {
  set.seed(2024)
  W_time_varying_list = lapply(seq(1:t), function(x) {W_USA})
  ...
  longitudinal_fitted_model <- community_interaction_GNARenet(simulate_longitudinal_data, usa_net_GNAR, alpha_order, beta_orders, W_USA,
                                                      list(red_states, blue_states, swing_states), interaction_order)
  
  
  ls_fit <- community_interaction_gnar_fit(simulate_longitudinal_data, usa_net_GNAR, alpha_order, beta_orders, W_USA,
                                           list(red_states, blue_states, swing_states), interaction_order)
  ls_fit_res_mat = residuals_matrix(ls_fit$residuals, 51)
  ls_fit_covariance_estimator = GNARcovariance_estimator_cpp(ls_fit_res_mat)
  GNARcovariance_logdet(ls_fit_covariance_estimator)
  GNAR_AIC(ls_fit$residuals, 51, 35)
  GNAR_BIC(ls_fit$residuals, 51, 35)
  GNAR_HQC(ls_fit$residuals, 51, 35)
  # red states community l2-error bound
  l2_errors[1, i] = sqrt(sum((coef(longitudinal_fitted_model, s = 0.2)[2:14] - theta0[1:13])^2))
  # blue states l2-error bound
  l2_errors[2, i] = sqrt(sum((coef(longitudinal_fitted_model, s = 0.2)[15:26] - theta0[14:25])^2))
  # swing states l2-error bound
  l2_errors[3, i] = sqrt(sum((coef(longitudinal_fitted_model, s = 0.2)[27:36] - theta0[26:35])^2))
  # complete model l2-error bound
  l2_errors[4, i] = sqrt(sum((coef(longitudinal_fitted_model, s = 0.2)[2:36] - theta0)^2))
  if (i %% 10 == 0) {
